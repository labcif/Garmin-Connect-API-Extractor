        ...
        if use_existing_token.lower() == "y":
            ...
            for line in output.splitlines():
                if line.startswith('Authorization: Bearer'):
                    ...
                    if not found:
                        print(Bcolors.OKGREEN + "[SUCCESS] Bearer token found!")
                        found = True
            if not found:
                print(Bcolors.FAIL + "[ERROR] Berar Token was not present! Please open Garmin Connect if possible to generate it.")
                sys.exit()
